...
if int(args.alg) == 1:
  ...
  bench_file =\
  "bench-"+args.rowsa+'-'+args.colsa
  f = open(bench_file,"w")

  strstr = '/home/eder/repos/calu2.15b/extern_test/mkl_dgetrf '+args.rowsa+' '+args.colsa
  ...
  for i in threads:
    print(strstr+' '+str(i)+' 1 >> '+bench_file+'...')
    os.system(strstr+' '+str(i)+' 1 >> '+bench_file)
...
if int(args.alg) == 2:
  ...
  for k in range(0,int(args.count)+1):
    strstr = '/home/eder/repos/calu2.15b/extern_test/mkl_dgetrf '+str(rowSizes[k])+' '+str(colSizes[k])+' '+\
    str(max_threads)+' 1'+' >> '+bench_file
...
if args.plot:
  ...
  if int(args.alg) == 2:
    if int(args.inc) == 0:
      ...
      'Threads: '+str(max_threads), fontsize=8)
    else:
      pl.title('Tiled GEP double Matrix dimensions: '+dimensions[0]+
