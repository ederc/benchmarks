...
if int(args.matrixsize) != 1:
  for file in os.listdir('.'):
    if fnmatch.fnmatch(file, 'test-'+args.matrixsize+'*-'+args.layout):
      benched.append(file)
else:
  for file in os.listdir('.'):
    if fnmatch.fnmatch(file, 'test-*-'+args.layout):
      benched.append(file)
...
if args.plot:
  ...
  markers = [\
  'None','None','None','None','None','None','None','None','o','None',\
  ...
  if int(args.matrixsize) != 1:
    pdfname =\
    'results-calu-mkl-inc-threads-'+str(args.matrixsize)+'-'+str(args.matrixsize)+\
    '-l'+args.layout+'.pdf'
  else:
    pdfname =\
    'results-calu-mkl-inc-matrices-'+str(args.threads)+'-l'+args.layout+'.pdf'
  pp =\
  PdfPages(pdfname)
  ...
  if int(args.matrixsize) != 1:
    ...
    ' x '+dimensions[1]+', Layout: '+args.layout+' (B ~ Blocksize)\n'+subtitle, fontsize=8)
  else:
    pl.title('Number of threads: '+str(args.threads)+', Layout: '+args.layout+' (B ~ Blocksize)\n'+subtitle, fontsize=8)
  if int(args.matrixsize) != 1:
